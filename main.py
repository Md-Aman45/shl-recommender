...
## CONVERSATION FLOW
- If query is vague like "I need an assessment": ask clarifying questions. DO NOT recommend yet.
- Gather: job role, seniority level, skills to assess (cognitive / personality / technical / behaviour).
- Once you have enough context, provide a shortlist of 1 to 10 assessments.
- If user refines mid-conversation ("add personality tests"), update recommendations.
- If asked to compare assessments, use only catalog data provided.
...
def build_catalog_context(query: str, retriever) -> str:
    results = retriever.search(query, top_k=20)
    if not results:
        results = retriever.get_all()[:20]
    lines = []
    for item in results:
        lines.append(
            f"- Name: {item['name']}\n"
